...
def bullish_fvg_respected(highs: List[float], lows: List[float], closes: List[float], opens: List[float]) -> bool:
    ...
    # Find if after FVG, price had at least one candle body inside zone, then closed above gap_top with bullish body
    for i in range(n+1, len(closes)):
        body_low = min(opens[i], closes[i])
        body_high = max(opens[i], closes[i])
        if gap_bottom <= body_low and body_high <= gap_top:
            # Now see if after that price closed above gap_top AND bullish candle
            for j in range(i+1, len(closes)):
                if closes[j] > gap_top and closes[j] > opens[j]:
                    return True
    return False
...
# Main scan function: bullish FVG respected only!
async def get_bullish_fvg_respect_coins(tf: str, top_n: int = 20) -> List[Tuple[str, float, float]]:
    ...
    for sym in symbols:
        try:
            ...
            if len(c) < 35:
                continue
            if bullish_fvg_respected(h, l, c, o):
...
async def fvg_coinlist_handler(update: TGUpdate, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    m = re.match(r'^(1H|4H|1D)\s*FVG\s*COIN\s*LIST\s*$', text, re.IGNORECASE)
    if not m:
    ...
    await update.message.reply_text(f"⏳ Scanning Binance ({tf_label}) Bullish FVG respected coins...", parse_mode=ParseMode.MARKDOWN)
    try:
        coins = await asyncio.to_thread(get_bullish_fvg_respect_coins, tf, 20)
        if not coins:
            await update.message.reply_text("😔 No bullish FVG respected coins found.", parse_mode=ParseMode.MARKDOWN)
            return
        lines = [f"🎯 Top Bullish FVG Respected Coins ({tf_label}) [Sorted: Volatility+Volume]:", ""]
